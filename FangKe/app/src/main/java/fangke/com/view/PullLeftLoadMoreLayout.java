...
/**
 * 左拉实现加载更多的自定义布局 继承自CoordinatorLayout
 */

public class PullLeftLoadMoreLayout extends CoordinatorLayout {
    private LoadingView mLoadingView;//加载更多的自定义view
    private View mList;
    ...
    public PullLeftLoadMoreLayout(Context context, AttributeSet attrs) {
        ...
        //字符串数组里面存的是查看更多跟 加载更多两个词语
        mTxtRes = new String[]{context.getString(R.string.txt_normal), context.getString(R.string.txt_chage)};

        mValueAnimator = ValueAnimator.ofFloat(0, 1);//传入线性参数
        mValueAnimator.setDuration(500);//传入动画持续时间
        mValueAnimator.addListener(new AnimatorListenerAdapter() {//设置动画侦听事件
    ...
    public void addView(int loadingHeight) {
        ...
        addView(mLoadingView, layoutParamsLoading);//添加加载更多的view 为其设置好id 参数
        ...
        addView(mTxt, layoutParams);//添加在加载更多上面字的textview 设置好参数
        mList = getChildAt(0);//其实也就是自定义布局中的第一个直接子view 也只有一个直接的子view
    }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (mList.getWidth() == getWidth() && dx > 0 && !ViewCompat.canScrollHorizontally(mList, ViewCompat.SCROLL_INDICATOR_LEFT) || dx < 0 && mList.getTranslationX() < 0) {
            //当mlist 也就是你要滑动的view 到了尽头 不能左移动的时候 此时继续往左滑动 就是我们显示加载更多的时机
            consumed[0] = dx;//全部消耗掉
            int distance = dx / 2;
